...
function getFileBirthTime(url) {
  ...
  try {
    const infoStr = (_a = spawnSync("git", ["log", "-1", '--pretty="%ci"', url]).stdout) == null ? void 0 : _a.toString().replace(/["']/g, "").trim();
    if (infoStr) {
...
function getPostsData(srcDir, config, ops) {
  return __async(this, null, function* () {
    ...
    for (const file of files) {
      ...
      frontmatter.cover = frontmatter.cover || ((_c = fileContent.match(/[!]\[.*?\]\((https:\/\/.+)\)/)) == null ? void 0 : _c[1]) || "";
      const html = mdRender.render(fileContent);
      const url = config.site.base + normalizePath(path2.relative(config.srcDir, file)).replace(/(^|\/)index\.md$/, "$1").replace(/\.md$/, config.cleanUrls ? "" : ".html");
      posts.push({
        filepath: file,
        fileContent,
        html,
...
function genFeed(config, rssOptions) {
  return __async(this, null, function* () {
    ...
    if (!rssOptions)
      return;
    const srcDir = config.srcDir.replace(config.root, "").replace(/^\//, "") || ((_a = process.argv.slice(2)) == null ? void 0 : _a[1]) || ".";
    ...
    for (const post of posts) {
      ...
      feed.addItem({
        ...
        image: frontmatter == null ? void 0 : frontmatter.cover,
        date: new Date(date)
      });
    }
    const RSSFilename = filename || "feed.rss";
    const RSSFilepath = path2.join(config.outDir, RSSFilename);
